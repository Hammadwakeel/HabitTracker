  ...
  return (
    <Tab.Navigator
      screenOptions={({ navigation }) => ({
        ...
        tabBarItemStyle: { width: 72, paddingHorizontal: 2 },
...
function AppNavigator({ onboardingDone, onOnboardingComplete }) {
  ...
  useEffect(() => { if (onboardingDone) scheduleNotifications(); }, [onboardingDone]);
...
function AppWithTheme() {
  ...
  useEffect(() => { getOnboarded().then(v => setOnboardingDone(!!v)); }, []);
  ...
  return <AppNavigator onboardingDone={onboardingDone} onOnboardingComplete={() => setOnboardingDone(true)} />;
}

export default function App() {
  return <ThemeProvider><AppWithTheme /></ThemeProvider>;
}

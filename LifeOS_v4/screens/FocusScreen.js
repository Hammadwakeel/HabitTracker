...
import { getTodayFocus, saveTodayFocus, getProfile } from '../utils/storage';

const DEFAULT_PROJECTS = ['Learning / Courses', 'Freelance / Upwork', 'Other'];
  ...
  useFocusEffect(useCallback(() => {
    (async () => {
      ...
      if (profile?.projects?.length > 0) {
        setProjects([...profile.projects.map(p => p.name), 'Other']);
      }
    })();
  }, []));

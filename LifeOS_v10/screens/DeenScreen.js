...
import {
  getTodayDeen, saveTodayDeen, getDeenStreak, getLast7DaysDeen, getProfile,
  getCachedPrayerTimes, cachePrayerTimes
} from '../utils/storage';
...
export default function DeenScreen() {
  ...
  const fetchPrayerTimes = async () => {
    ...
    try {
      ...
      if (status === 'granted') {
        ...
        // Check cache first
        const cached = await getCachedPrayerTimes(city);
        if (cached) { setPrayerTimes(cached); setLoadingTimes(false); return; }
        ...
        if (data.code === 200) {
          ...
          await cachePrayerTimes(city, times);
        }
      } else {
      ...
      const saved = await AsyncStorage.getItem('deenCity');
      if (saved) {
        const cached = await getCachedPrayerTimes(saved);
        if (cached) { setPrayerTimes(cached); setLocationName(saved + ' (cached)'); setLoadingTimes(false); return; }
      }
      setTimeError('No internet. Connect to load prayer times.');
  ...
  const fetchByCity = async (city) => {
    ...
    try {
      ...
      const cached = await getCachedPrayerTimes(city);
      if (cached) {
        setPrayerTimes(cached); setLocationName(city);
        setShowCityInput(false); setCityInput('');
        setLoadingTimes(false); return;
      }
      const today = new Date();
      const date = `${today.getDate()}-${today.getMonth() + 1}-${today.getFullYear()}`;
    ...
    } catch { setTimeError('No internet connection. Using cached times if available.'); }
    finally { setLoadingTimes(false); }
  };
  ...
  const onRefresh = async () => {
    ...
    setRefreshing(true);
    await Promise.all([
      getTodayDeen().then(d => setDeen(d)),
      getDeenStreak().then(s => setStreak(s)),
      getLast7DaysDeen().then(h => setHistory(h)),
    ]);
    setRefreshing(false);
  };
